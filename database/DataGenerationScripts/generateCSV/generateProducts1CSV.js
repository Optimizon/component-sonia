...
var fileName = '../fakeProductData/TESTproducts1.csv'  //uncomment to run

csvWrite.pipe(fs.createWriteStream(fileName, {'flags': 'a'}));

for (var i = 1 ; i < 5000001 ; i++) { //generates data points 1 to 5MILL.0
	let productObject =  
	[ i ,
	"shirt" + i, //productNamer
	randomDescription(), 
	color(), 
	randomPrice(),
	randomImage(),
...
//could run script in package.json || the following:
//node --max-old-space-size=8000 ./database/DataGenerationScripts/generateCSV/generateProducts1CSV.js && node --max-old-space-size=8000 ./database/DataGenerationScripts/generateCSV/generateProducts2CSV.js && node --max-old-space-size=8000 ./database/DataGenerationScripts/generateCSV/generateSimilar1CSV.js && node --max-old-space-size=8000 ./database/DataGenerationScripts/generateCSV/generateSimilar2CSV.js && node --max-old-space-size=8000 ./database/DataGenerationScripts/generateCSV/generateSimilar3CSV.js && node --max-old-space-size=8000 ./database/DataGenerationScripts/generateCSV/generateSimilar4CSV.js && node --max-old-space-size=8000 ./database/DataGenerationScripts/generateCSV/generateSimilar5CSV.js && node --max-old-space-size=8000 ./database/DataGenerationScripts/generateCSV/generateSimilar6CSV.js
